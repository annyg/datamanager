...
For users on a remote server without internet access to GitHub (but
with CRAN dependencies already available), `datamanager` can be
installed from a source tarball (`.tar.gz`) transferred to the machine.

1.  On a machine with internet access, download the source tarball from
    the [GitHub
    releases](https://github.com/annyg/datamanager/releases) page (file
    named `datamanager_<version>.tar.gz`), or build one from a clone of
    the repository:
